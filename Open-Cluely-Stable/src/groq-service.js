...
`;
  ...
  /**
   * Generates speech from text using Groq TTS API.
   * @param {string} text The text to convert to speech.
   * @param {string} voice The voice to use. Options: onyx, alloy, echo, fable, nova, shimmer.
   * @returns {Promise<string>} Base64 encoded audio data.
   */
  async generateSpeech(text, voice = 'onyx') {
    if (!text || text.trim() === '') {
      throw new Error('Groq TTS: No text provided');
    }

    // Supported voices by Groq/OpenAI-compatible endpoints
    const validVoices = ['onyx', 'alloy', 'echo', 'fable', 'nova', 'shimmer'];
    const selectedVoice = validVoices.includes(voice.toLowerCase()) ? voice.toLowerCase() : 'onyx';

    console.log(`GroqService: Fetching TTS for text "${text.substring(0, 30)}..." with voice ${selectedVoice}`);
    ...
    if (!res.ok) {
      ...
      console.error(`GroqService: TTS Error: ${errorMsg}`);
      throw new Error(`Groq TTS API Error: ${errorMsg}`);
    }

    const arrayBuffer = await res.arrayBuffer();
    console.log(`GroqService: Received audio buffer of size ${arrayBuffer.byteLength}`);
