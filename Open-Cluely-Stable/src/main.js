...
Analyze the screenshots and conversation context:`;
  ...
  ipcMain.handle('ask-ai', async (event, question, history) => {
    ...
    if (!geminiService || !geminiService.model) {
      return { success: false, error: 'AI not initialized. Check your GEMINI_API_KEY.' };
    }
    ...
    try {
      ...
      console.log('Asking Gemini (text-only)...');
      const answer = await geminiService.generateText(prompt);
      console.log('Gemini answered, length:', answer?.length);
      ...
      if (msg.includes('API_KEY') || msg.includes('API key')) msg = 'Invalid API key. Check GEMINI_API_KEY in .env';
      else if (msg.includes('quota')) msg = 'Gemini quota exceeded. Try again in a moment.';
      else if (msg.includes('404') || msg.includes('not found')) msg = 'Model not found. Check gemini-service.js model name.';
