...
// App switches must be set before app is ready
...
function resolveSttEngine() {
  ...
  const hasWhisper = fs.existsSync(isDevelopment() 
    ? path.join(__dirname, '..', 'faster_whisper_live.py')
    : path.join(process.resourcesPath, 'faster_whisper_live.py'));

  if (explicit === 'whisper' || explicit === 'faster-whisper' || (!explicit && hasWhisper)) {
...
Analyze the screenshots and conversation context:`;
  ...
  ipcMain.handle('generate-speech-groq', async (event, text, voice) => {
    console.log(`IPC: generate-speech-groq called, voice: ${voice}, text length: ${text?.length}`);
    try {
      let result;
      if (!geminiService || typeof geminiService.generateSpeech !== 'function') {
        console.log('IPC: Using standalone GroqService for TTS');
        const groqKey = String(process.env.GROQ_API_KEY || '').trim();
        if (groqKey) {
          const tempGroq = new GroqService(groqKey);
          const base64 = await tempGroq.generateSpeech(text, voice);
          result = { success: true, base64 };
        } else {
          result = { success: false, error: 'Groq TTS not available (check GROQ_API_KEY)' };
        }
      } else {
        console.log('IPC: Using active geminiService for TTS');
        const base64 = await geminiService.generateSpeech(text, voice);
        result = { success: true, base64 };
      }
      
      console.log(`IPC: generate-speech-groq success, result length: ${result.base64?.length}`);
      return result;
    } catch (error) {
      console.error('IPC: generate-speech-groq error:', error.message);
  ...
  ipcMain.handle('start-voice-recognition', () => {
    ...
    try {
      const _stt = resolveSttEngine();
      const scriptName = _stt.engine === 'whisper' ? 'faster_whisper_live.py' : 'vosk_live.py';
      
      const pythonScript = isDevelopment()
        ? path.join(__dirname, '..', scriptName)
        : path.join(process.resourcesPath, scriptName);
      console.log(`Starting ${_stt.engine} live transcription:`, pythonScript);
