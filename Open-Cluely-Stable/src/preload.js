...
try {
  contextBridge.exposeInMainWorld('electronAPI', {
    ...
    getSttConfig: () => {
      return ipcRenderer.invoke('get-stt-config').catch(err => ({
        engine: 'vosk',
        warning: err.message,
    ...
    transcribeAudioGroq: (base64, mimeType) => {
      return ipcRenderer.invoke('transcribe-audio-groq', base64, mimeType).catch(err => ({
        success: false,
        error: err.message
      }));
    },

    generateSpeechGroq: (text, voice) => {
      console.log('PreloadAPI: generateSpeechGroq called');
      return ipcRenderer.invoke('generate-speech-groq', text, voice).catch(err => ({
        success: false,
        error: err.message
      }));
    },

    // Ask AI a plain text question (voice-driven Q&A, no screenshots needed)
    askAI: (question, history) => {
      console.log('PreloadAPI: askAI called, question:', question?.substring(0, 60));
      return ipcRenderer.invoke('ask-ai', question, history).catch(err => {
        console.error('PreloadAPI: askAI error:', err);
    ...
    // Window resize – cycles presets or jumps to a named one ('small'|'medium'|'large'|'fullscreen')
    resizeWindow: (preset) => {
      console.log('PreloadAPI: resizeWindow called, preset:', preset);
      return ipcRenderer.invoke('resize-window', preset).catch(err => {
        console.error('PreloadAPI: resizeWindow error:', err);
        return { success: false, error: err.message };
      });
    },

    getWindowSize: () => {
      return ipcRenderer.invoke('get-window-size').catch(err => ({
        width: 900, height: 420
      }));
    },

    // Fired when the global Ctrl+Alt+Shift+R shortcut triggers a resize from main process
    onWindowResized: (callback) => {
      const handler = (event, presetLabel) => {
        try { callback(presetLabel); } catch (e) { console.error('onWindowResized cb:', e); }
      };
